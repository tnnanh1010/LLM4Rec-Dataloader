...
class BaseTrainer(ABC):
    ...
    def _init_logger(self):
        ...
        if self.args.mode == 'train':
            ...
            log.create_csv(self._eval_csv, header=['global_step'] + eval_header)
            self._eval_epoch = os.path.join(self._log_path, 'epoch.csv')
            log.create_csv(self._eval_epoch, header=['global_step', 'train_loss'] + eval_header)
    ...
    def _log_arguments(self):
        ...
        Returns:
            None
        """
        log.log_json(path=self._path, data=self.args, name='args')
    ...
    def _log_train(self, scheduler, loss, global_step, epoch):
        ...
        log.log_csv(self._loss_csv, data)
    ...
    def _log_epoch(self, train_loss: float, valid_loss: float, scores: dict, epoch: int):
        ...
        log.log_csv(self._eval_epoch, data)
    ...
    def _log_eval(self, global_step, loss: float = None, scores: dict = None):
        ...
        log.log_csv(self._eval_csv, data)
